...
test_that("Func returns correct limits", {
  expect_identical(find_alt_len(en_file), 100)
  expect_identical(find_alt_len(de_file), 115)
  expect_identical(find_alt_len(ko_file), 90)
  expect_identical(find_alt_len(us_file), 100)
  
})

test_that("Func returns NULL if no limit found", {
  expect_null(find_alt_len(cymru_file))
})
